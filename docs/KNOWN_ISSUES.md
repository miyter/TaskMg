...
| 項目 | 現状 | 対応 |
|------|------|------|
| i18n違反 | `confirm()`で日本語ハードコード | `t('msg.confirm_delete')`を使用し、可能ならモーダル確認UIに置換 |

#### SearchView.tsx
| 項目 | 現状 | 対応 |
|------|------|------|
| i18n違反 | 「でいつでも検索を開始」等がハードコード | `t('search.tips')`等を使用してローカライズ |
| 密度 | 標準的な余白 | **設計思想**: 検索時はより広大な余白（Spacious）をデフォルトに検討 |

#### Wizard / Wiki / Dashboard
| 項目 | 現状 | 対応 |
|------|------|------|
| i18n違反 | 「戻る」「次へ」「残り」「フレームワーク Wiki」等が日本語ハードコード | 全ての重要機能、特にウィザードと Wiki データの i18n 化完了が必要 |
| i18n違反 | ボタン title 属性等に英語がハードコード | `t()` を通じて多言語対応を一貫させる |
...
| カテゴリ | 課題内容 | 影響 |
|----------|----------|------|
| **アトミックデザイン** | `Button`, `Input` 等の汎用的な「原子」コンポーネントが不足。 | 各所で Tailwind クラスが重複定義されており、デザイン変更のコストが高い。 |
| **アイコン管理** | `Icons.tsx` が一部の機能のみで、多くがハードコード。 | テーマ変更やアイコンセットの一括置換が困難。 |
| **UI実装の不整合** | `TaskDetailModal`, `WizardStep`, `SettingsModal` 等で `confirm()` / `alert()` を多用。 | プレミアムな体験のため、独自モーダルUIへの置換が必要。 |
| **タイムアウト制御** | `useFirestoreSubscription` のデータ待機が 10 秒固定。 | プレミアムな体験の阻害。状況に応じた動的な制御や UI 表現の工夫が必要。 |
