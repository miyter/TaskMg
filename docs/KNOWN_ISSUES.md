...
- **モーダルデザインの最適化**: 現状、デスクトップ環境でもセンター配置の大型モーダルが主体となっている。デスクトップ向けにはサイドパネル（Drawer）形式や、よりコンパクトなUIへの最適化を検討する。
- **ブラウザ標準ダイアログの排除**: `SettingsModal.tsx` 等で依然として `alert()` や `confirm()` が使用されている箇所がある。これらを設計思想に基づき、カスタムモーダルまたはトースト通知に置き換える。
