...
- **多言語対応 (i18n)**: 日本語・英語の切り替えに対応。UIの大部分でローカライズ済み（バリデーションエラーメッセージ等の一部は未対応）。
...
残存課題の詳細は `docs/KNOWN_ISSUES.md` を参照。

---

## 🚨 インシデント記録 (Incidents)

- **2026-01-05: Zod バリデーションクラッシュ & 無限ループ対策**
    - **事象**: ビルド後の環境で `Uncaught TypeError: Cannot read properties of undefined (reading 'defaultError')` が発生し、操作時にアプリがクラッシュする状態となっていた。
    - **原因**: `Zod` のカスタムエラーマップ関数内で、コンテキストオブジェクト `ctx` が `undefined` になるケース（バリデーションエラー時）のハンドリングが不足しており、プロパティアクセス時に例外が送出されていた。
    - **対応**:
        1. **Fail-safe 実装**: `ctx` の存在チェックを追加し、フォールバックメッセージを表示するよう修正。
        2. **安全性向上**: 同種のエラーによる「再レンダリング無限ループ」を防ぐため、`ErrorBoundary` に「緊急リセット（Hard Reset）」ボタンを追加。キャッシュやローカル設定が破損してループした場合でも、ユーザー自身で復旧できる手段を確保した。
