        ...
        <div className="max-w-4xl mx-auto relative pb-10">
            ...
            {data.milestones.map((item, index) => {
                ...
                if (isGoal) {
                    ...
                    icon = <div className="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center text-xs font-bold shadow-md ring-4 ring-gray-50 dark:ring-gray-900 z-10 animate-pulse relative">{t('target_dashboard.backward.now_icon')}</div>;
                        ...
                        <div className="flex-1 pr-4 min-w-0">
                            <div className={`px-3 py-3 rounded-lg border ${isCurrent && !contentClass.includes('border-2') ? 'border-2 border-blue-500' : isCurrent ? '' : 'border-gray-200 dark:border-gray-700'} ${contentClass} shadow-sm transition-all hover:shadow-md hover:translate-x-1 cursor-pointer`}>
                                <div className="flex items-center justify-between gap-3">
                                    ...
                                    {isGoal && <span className="text-[10px] bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full font-bold flex-shrink-0">{t('target_dashboard.backward.target_label')}</span>}
                                    {isCurrent && <span className="text-[10px] text-blue-500 font-bold flex-shrink-0">{t('target_dashboard.backward.now_label')}</span>}
                                </div>
                            </div>
                        </div>
                    </div>
