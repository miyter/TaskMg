...
import { ASSETS, DOM_IDS } from './ui-view-constants.js';
...
function showCriticalError(error) {
    ...
    overlay.innerHTML = `
        ...
        <code style="background: #333; padding: 10px; border-radius: 4px; font-size: 12px;">${error.message}</code>
        <button onclick="location.reload()" style="margin-top: 20px; padding: 10px 20px; cursor: pointer;">再読み込み</button>
    `;
...
async function startApplication() {
    ...
    try {
        setupHeadElements();
        // initializeAppがPromiseを返す可能性を考慮しawait
        await initializeApp();
    } catch (error) {
