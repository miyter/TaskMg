...
function createSectionHeader(title, targetId, hasAddButton = false, buttonId = '') {
    ...
    const addButton = hasAddButton ? `
        <button id="${buttonId}" class="text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700 flex items-center justify-center" title="${title}を追加" aria-label="${title}を追加">
    ...
    return `
        <div class="flex items-center justify-between px-3 py-2 group cursor-pointer sidebar-section-header" 
            ...
            <div class="flex items-center min-w-0">
                <svg class="w-3 h-3 text-gray-400 mr-2 transition-transform duration-200 flex-shrink-0" style="${rotateStyle}" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
...
export function setupSidebarToggles() {
    document.querySelectorAll('.sidebar-section-header').forEach(header => {
        ...
        header.addEventListener('click', (e) => {
            ...
            const isHidden = list.classList.contains(SIDEBAR_CONFIG.CLASSES.HIDDEN);
            const newState = isHidden;

            list.classList.toggle(SIDEBAR_CONFIG.CLASSES.HIDDEN, !newState);
            icon.style.transform = newState ? 'rotate(0deg)' : 'rotate(-90deg)';
            header.setAttribute('aria-expanded', newState);

            localStorage.setItem(getSectionKey(targetId), newState);
        });
    });
}