 ...
 */
export function renderInlineInput(container: HTMLElement, projectId: string | null, labelId: string | null, options: InlineInputOptions = {}): void {
    const isExpanded = options.forceExpand || container.dataset.expanded === 'true';

    if (!isExpanded) {
        container.innerHTML = buildInlineButtonHTML();
        const btn = container.querySelector('#show-input-btn') as HTMLElement;
        if (btn) {
            btn.onclick = () => {
 ...
 */
export function renderFixedAddTaskBar(footerContainer: HTMLElement, inputContainer: HTMLElement | null, projectId: string | null, labelId: string | null): void {
    footerContainer.innerHTML = buildFixedButtonHTML();

    // ボタンクリックでタスクモーダルを開く
    const btn = footerContainer.querySelector('#fixed-add-task-btn') as HTMLElement;
    if (btn) {
        btn.onclick = () => {
            // タスクモーダルを開く（新規作成モード）
...
function setupStandaloneEvents(): void {
    ...
    form.onsubmit = async (e) => {
        ...
        const dateInput = document.getElementById('task-due-date-input') as HTMLInputElement;
        const dateVal = dateInput.value;
        const dueDate = dateVal ? new Date(dateVal) : null;

        if (dateVal && dueDate && isNaN(dueDate.getTime())) {
            return showMessageModal({ message: "無効な日付だ", type: 'error' });
        }

        const projectInput = document.getElementById('task-project-input') as HTMLSelectElement;
...
function setupInlineEvents(container: HTMLElement, projectId: string | null, labelId: string | null, options: InlineInputOptions = {}): void {
    const titleInput = container.querySelector('#inline-title-input') as HTMLInputElement;
    const submitBtn = container.querySelector('#submit-task-btn') as HTMLButtonElement;
    const cancelBtn = container.querySelector('#cancel-input-btn') as HTMLButtonElement;
    const descInput = container.querySelector('#inline-desc-input') as HTMLTextAreaElement;
    const tbSelect = container.querySelector('#inline-timeblock-select') as HTMLSelectElement;
...
function buildStandaloneFormHTML(projects: Project[]): string {
...
function buildInlineButtonHTML(): string {
    return `
...
function buildInlineFormHTML(timeBlocks: TimeBlock[]): string {
...
function buildFixedButtonHTML(): string {
    return `
