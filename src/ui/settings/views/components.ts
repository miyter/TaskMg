...
export function createAccordionSection(id: string, title: string, icon: string, content: string, isOpen: boolean = false): string {
    return `
...
export function createRadioOption(name: string, value: string, label: string, isChecked: boolean, icon: string = ''): string {
    return `
...
export function createSelectOption(label: string, options: SelectOption[], selectedValue: string, name: string): string {
    const optionsHtml = options.map(opt =>
        `<option value="${opt.value}" ${String(opt.value) === selectedValue ? 'selected' : ''}>${opt.label}</option>`
    ).join('');
