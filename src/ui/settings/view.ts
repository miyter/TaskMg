...
import { getAccountContent } from './views/account';
import { BACKGROUND_PATTERNS, getAppearanceContent } from './views/appearance';
import { createAccordionSection } from './views/components';
import { getDataContent } from './views/data';
import { FONT_SIZES, getFontSizeContent, getTypographyContent } from './views/typography';

import { getLanguageContent } from './views/language';

// エクスポート: 定数はここから再エクスポート
export { BACKGROUND_PATTERNS } from './views/appearance';
export { FONT_SIZES } from './views/typography';

export function getSettingsModalHTML(userInitial: string | null, userEmail: string | null, isCompact: boolean): string {
    ...
    const currentLang = localStorage.getItem('ui_language') || 'ja';

    // 各セクションのコンテンツ生成
    const appearanceContent = getAppearanceContent(currentTheme, currentBg, currentDensity, isCompact);
    const languageContent = getLanguageContent(currentLang);
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl ${UI_STYLES.MODAL.WIDTH.DEFAULT} overflow-hidden flex flex-col max-h-[90vh] h-full sm:h-auto animate-fade-in-up">
            ...
            <div class="overflow-y-auto custom-scrollbar flex-1 bg-white dark:bg-gray-800">
                <div class="p-6 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                    <!-- Left Column -->
                    <div class="space-y-0">
                        ${createAccordionSection('appearance', 'Appearance', '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01"></path></svg>', appearanceContent)}
                        ${createAccordionSection('language', 'Language', '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129"></path></svg>', languageContent)}
