...
// @ts-ignore: Constants file might be missing ASSETS.FAVICON in TS version or path issue. Checking layout constants.
...
function showCriticalError(error: Error): void {
    ...
    // インライン属性を廃止し、リスナーを登録
    const reloadBtn = overlay.querySelector('#error-reload-btn') as HTMLElement;
    if (reloadBtn) {
 ...
 * ブラウザ環境のヘッド要素を最適化
 */
function setupHeadElements(): void {
    // 既存のアイコンをクリーンアップ
