 ...
 * 他のブロックとの重複チェック
 */
export function checkOverlap(currentId: string | null, startMin: number, endMin: number): boolean {
 ...
 * 追加ボタンの状態を更新
 */
export function updateAddButtonUI(btn: HTMLButtonElement | null, count: number): void {
    if (!btn) return;
