...
// Helper interface for rendering row data
export interface TimeBlockRowData {
    id: string | null;
    start: string;
    end: string;
    color: string;
...
const generateOptions = (options: string[], selected: string) =>
...
function buildCustomSelect(options: string[], value: string, type: string) {
...
export function buildRowHTML(data: TimeBlockRowData) {
