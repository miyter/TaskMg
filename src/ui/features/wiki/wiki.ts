 ...
 * フレームワーク詳細モーダルを表示
 */
function showFrameworkModal(framework: WikiFramework): void {
    ...
    document.getElementById('framework-modal-overlay')?.addEventListener('click', (e) => {
        if ((e.target as HTMLElement).id === 'framework-modal-overlay') {
            closeModal();
        }
    });
    ...
    document.getElementById('apply-framework-btn')?.addEventListener('click', (e) => {
        const modeId = (e.target as HTMLElement).dataset.mode;
        closeModal();
